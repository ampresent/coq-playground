...
(** r the last chapter, we've been working with lists
...
Check @combine.
Compute (combine [1;2;3] [false;false;true;true]).
...
Definition filter_even_gt7 (l : list nat) : list nat :=
  filter (fun n => negb (n <=? 7)) (filter even l).
...
Lemma map_two_lists : forall (X Y : Type) (f : X -> Y) (l1 l2 : list X),
  map f (l1 ++ l2) = (map f l1) ++ (map f l2).
Proof.
  intros t1 t2 f l1 l2.
...
Proof.
  ...
  - simpl. reflexivity.
  - simpl. rewrite <- IHl'. rewrite map_two_lists. reflexivity.
Qed.
...
Compute prod_uncurry app ([1;2;3], [1;2;3]).
Compute prod_curry (prod_uncurry app) [1;2;3] [1;2;3].
...
(** **** Exercise: 2 stars, advanced (nth_error_informal)
   ...
   Suppose l = [], then we easily get n = 0 and @nth_error X [] 0 = None.

   Suppose l != [], then l can be represented as h::l'.
   Then we know length l = length l' + 1
   @nth_error X l n = @nth_error X (_::l1) (n1-1).
   We apply the definition recursively,
   @nth_error X l n = @nth_error X (_::l2) (n2-2).
   Until we get,
   @nth_error X l n = @nth_error X (_::li) 1.
   Where i = (length l - 1).
   Finally we apply the definition one more time. We get,
   @nth_error X l n = @nth_error X [] 0 = None.
   Qed.
*)
...
Fail Compute three cnat (fun n => mult n 2) 1.
Compute three nat (fun n => n * 2) 1.
