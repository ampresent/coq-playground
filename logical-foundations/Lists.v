...
Proof. simpl subset. reflexivity. Qed.
...
Lemma equal_lemma : forall (n : nat),
  (n=?n) = true.
Proof.
  intros n.
...
Proof.
  ...
  induction l as [|n l' IHl'].
  - simpl. reflexivity.
  - simpl. rewrite equal_lemma. rewrite IHl'. reflexivity.
Qed.
...
Theorem bag_count_sum : forall (b1 : bag) (b2 : bag),
  count 0 (sum b1 b2) = (count 0 b1) + (count 0 b2).
Proof.
  intros b1 b2.
  induction b1 as [|[] t IHb'].
  - simpl. reflexivity.
  - simpl. rewrite IHb'. reflexivity.
  - simpl. rewrite IHb'. reflexivity.
Qed.
...
(** 0 elements. baz is a recursive inductive type with no limit. *)
